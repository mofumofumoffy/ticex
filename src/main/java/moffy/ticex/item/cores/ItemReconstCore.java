...
public class ItemReconstCore extends Item {
    ...
    public void appendHoverText(ItemStack stack, Level level, List<Component> components, TooltipFlag tooltipFlag) {
        if (modifierName == null) {
            components.add(
                Component.translatable("item." + TicEX.MODID + ".reconstruction_core.desc").withStyle(
                    ChatFormatting.GRAY
                )
            );
        } else if (applyLevel) {
            components.add(
                Component.translatable("modifier." + TicEX.MODID + "." + modifierName + "." + modifierLevel).withStyle(
                    ChatFormatting.AQUA
                )
            );
        } else {
            components.add(
                Component.translatable("modifier." + TicEX.MODID + "." + modifierName).withStyle(ChatFormatting.AQUA)
            );
        }
    }
}

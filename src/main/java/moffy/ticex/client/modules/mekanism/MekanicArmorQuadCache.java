...
public class MekanicArmorQuadCache {
    ...
    public static final Table<EquipmentSlot, ModuleData<?>, ModuleModelSpec> moduleModelSpec = HashBasedTable.create();
    public static final Map<UUID, BoltRenderer> boltRenderMap = new Object2ObjectOpenHashMap<>();

    protected static final Map<EquipmentSlot, LoadingCache<QuickHash, ArmorQuads>> quadCacheMap = new HashMap<>();

    public static void createQuadCache(EquipmentSlot slotType){
        if(!quadCacheMap.containsKey(slotType)){
            LoadingCache<QuickHash, ArmorQuads> cache = CacheBuilder.newBuilder()
                    .build(
                            new CacheLoader<>() {
                                @NotNull
                                @Override
                                ...
                                public ArmorQuads load(@NotNull QuickHash key) {
                                    return createQuads(
                                            slotType,
                                            resolveAdjacentEquipmentType(slotType),
            ...
            quadCacheMap.put(slotType, cache);
            MekaPlateModelCache.INSTANCE.reloadCallback(cache::invalidateAll);
        }
    }

    public static LoadingCache<QuickHash, ArmorQuads> getOrCreateCache(EquipmentSlot slot){
        if(!quadCacheMap.containsKey(slot)){
            createQuadCache(slot);
        }
        return quadCacheMap.get(slot);
    }
    ...
    private static EquipmentSlot resolveAdjacentEquipmentType(EquipmentSlot slot){
        return switch (slot){
    ...
    private static ArmorQuads createQuads(
            EquipmentSlot type,
            EquipmentSlot adjacentType,
    ...
    static String processOverrideName(String part, String name) {
    ...
    public record ModuleModelSpec(
        ...
        public String processOverrideName(String part) {
            return MekanicArmorQuadCache.processOverrideName(part, name);
        }
    }
        ...
        protected void reload(ModelEvent.BakingCompleted evt) {
            ...
            for (String name : getModel().getRootComponentNames()) {
                ...
                if (matchingSpec != null) {
                    SpecData specData = specParts.computeIfAbsent(matchingSpec, spec ->
                            new SpecData(new HashSet<>(), new HashSet<>())
                    );

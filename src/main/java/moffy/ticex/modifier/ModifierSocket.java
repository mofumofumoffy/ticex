...
public class ModifierSocket
    ...
    public Component validate(IToolStackView tool, ModifierEntry entry) {
        if (tool.getModifierLevel(TicEXModifiers.PSIONIZING_RADIATION_MODIFIER.get()) < 1) {
    ...
    public List<ModifierEntry> displayModifiers(ModifierEntry entry) {
        return List.of(new ModifierEntry(TicEXModifiers.PSIONIZING_RADIATION_MODIFIER.get(), 1));
    }
}

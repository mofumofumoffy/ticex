...
public class TicEXCCEvent {
        ...
        if (!event.getEntity().level().isClientSide) {
            ...
            if (hasModem(player)) {
                ...
                if (computerItem != null) {
                    computerItem.queueEvent(
                        "player_attack",
                        new Object[] {
                            TicEXCCUtils.createEntityMapWithProps(player),
                            TicEXCCUtils.createEntityMap(target),
                        }
                    );
                }
            }
            ...
            if (target instanceof Player player && hasModem(player)) {
                ...
                if (computerItem != null) {
                    computerItem.queueEvent(
                        "player_hurt",
                        new Object[] {
                            TicEXCCUtils.createEntityMap(source),
                            TicEXCCUtils.createEntityMapWithProps(player),
                        }
                    );
                }
    ...
    public static boolean hasModem(Player player) {
        ...
        if (chestStack.getItem() instanceof IModifiable) {
            ...
            return chest.getModifierLevel(TicEXModifiers.MODEM_MODIFIER.get()) > 0;
        }
        return false;
    }
